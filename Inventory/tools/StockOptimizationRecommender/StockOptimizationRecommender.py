...
def optimize_stock_levels(
    ...
    try:
        ...
        if inventory_data.empty or sales_data.empty:
            ...
            logger.warning("No data found in the actual database. Using generated sample data as a fallback ONLY for demonstration.")
            inventory_data, sales_data = generate_sample_data()
            analysis_result = "Using generated sample data for demonstration purposes only. This is a fallback mechanism when no actual data is available.\n\n"
        else:
            logger.info(f"Using ACTUAL inventory data from database: {len(inventory_data)} records")
            logger.info(f"Using ACTUAL sales data from database: {len(sales_data)} records")
